...
def feature_prediction(x_train, y_train, x_test, y_test, n_voxel=500, n_iter=200):
    ...
    for epoch in range(epochs):
        ...
        for i, (input, target) in enumerate(train_dataloader):
            ...
            y_true_list.append(target.detach().numpy())
            y_pred_list.append(y_pred.detach().numpy())

            print("Loss: ", loss)

        save_path = "model_weight_"+str(epoch)+".pth"
    ...
    for i, (input, target) in enumerate(test_dataloader):
        ...
        test_true_list.append(target.detach().numpy())
        test_pred_list.append(y_pred.detach().numpy())
